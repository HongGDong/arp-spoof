...
int get_wlan_ip(const char* ifname, char* ip) {
...
int get_wlan_mac(const char* ifname, unsigned char* mac) {
    struct ifaddrs* ifaddr, * ifa;
...
int relay_packet(const u_char* packet, const char* ifname, const char* s_addr, const char* d_addr) {
    ...
    // Replace header->len with the actual length of the packet
    // Here we need to know the packet length; assuming you have this information
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < s_index; i++) {
        IP_GATE.insert({ sender[i], target[i] });
    ...
    while (true) {
        ...
        else if (eth->type() == 0x0800) { // IP packet
